...
namespace Website.Models
{
	public class Case_information_db
	{
		...
		public void Update_case_information(Case_informatio case_Information, string uername)
		{

			select_indect select = new select_indect();
			SqlConnection sqlConnection = new SqlConnection(ConnStr);
			SqlCommand sqlCommand = new SqlCommand(
				@"update case_Informatio set name = @name,six = @six,birthday = @birthday,[identity] = @identity,telephone = @telephone,economic = @economic,obstacleclass = @obstacleclass,obstacledegree = @obstacledegree,contact = @contact,phone = @phone,notedate = @notedate,CMS = @CMS,BA1project = @BA1project,BA2project = @BA2project,BA3project = @BA3project,BA4project = @BA4project,
				BA5project = @BA5project,BA6project = @BA6project,BA7project = @BA7project,BA8project = @BA8project,BA9project = @BA9project,BA10project = @BA10project,BA1amount = @BA1amount,BA2amount = @BA2amount,BA3amount = @BA3amount,BA4amount = @BA4amount,BA5amount = @BA5amount,BA6amount = @BA6amount,BA7amount = @BA7amount,BA8amount = @BA8amount,BA9amount = @BA9amount,BA10amount = @BA10amount,nateunit = @nateunit,waiter = @waiter,status = @status,no_service_reason = @no_service_reason,no_service_date = @no_service_date, BAnum = @BAnum
				where [index] = @index");
			sqlCommand.Connection = sqlConnection;
			sqlCommand.Parameters.Add(new SqlParameter("@name", case_Information.name));
			sqlCommand.Parameters.Add(new SqlParameter("@six", case_Information.six));
			...
			if (case_Information.no_service_date == null)
				...
				sqlCommand.Parameters.Add(new SqlParameter("@no_service_date", case_Information.no_service_date));
			sqlCommand.Parameters.Add(new SqlParameter("@BAnum", case_Information.BAnum));
			sqlCommand.Parameters.Add(new SqlParameter("@index", case_Information.index));
			sqlConnection.Open();
			sqlCommand.ExecuteNonQuery();
			sqlConnection.Close();
		}
		...
		public List<Case_informatio> Get_Case_informatio(string status)
		{
			...
			if (reader.HasRows)
			{
				while (reader.Read())
				{
					...
					{
						index = reader.GetInt32(reader.GetOrdinal("index")).ToString(),
		...
		public List<Case_informatio> Get_One_Case_informatio(string id)
		{
			List<Case_informatio> case_information = new List<Case_informatio>();
			SqlConnection sqlConnection = new SqlConnection(ConnStr);
			SqlCommand sqlCommand = new SqlCommand("SELECT * FROM case_informatio where index = '" + id + "'");
			sqlCommand.Connection = sqlConnection;
			sqlConnection.Open();
			SqlDataReader reader = sqlCommand.ExecuteReader();
